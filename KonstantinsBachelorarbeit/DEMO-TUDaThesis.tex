...
Mein Beitrag.
Warum Javascript, warum OpenGL? Was wären die Alternativen gewesen?


%Ursprünglich habe ich angefangen den visualisierer in python zu schreiben. 

Warum Javascript, warum OpenGL?
Der Visualisierer soll später mit GAALOPWeb zusammen genutzt werden können. Deshalb habe ich mich entschieden, das ganze als Webseite zu schreiben (also HTML und JavaScript). Davor hatte ich schon Experimente zur Visualisierung in Python gemacht wobei ich OpenGL, Numpy und Pyvista genutzt hatte. Deshalb habe ich mich entschieden WebGL2 zu nutzen als Alternative zu OpenGL.
...
Da das Verfahren eine lineare Näherung nutzt, ist es nur lokal konvergent. Deshalb lohnt es sich eine maximale Schrittweite einzuführen. Diese kann die Stabilität des Verfahrens verbessern.
...
Meine erste Idee zur Visualisierung von Geometric Algebra war Marching Cubes zu benutzen. 
Marching Cubes ist ein häufig genutzter Algorithmus zur Isoflächenerzeugung der den Vorzeichen Übergang von Funktionen ausnutzt. Leider gibt es in f keinen Vorzeichen Übergang, da \(f\) immer positiv ist. 

Deswegen habe ich eine modifizierte Variante des Algorithmus implementiert.
...
In Theorie könnte man P zu einer Polynombasis machen, um somit ein kleineres Gleichungssystem zu erhalten.  Wenn die Polynome aus \(\texttt{point}(x,y,z)\) bekannt sind, könnte man eine maximale linear unabhängige Untermenge der Polynome als Basis wählen. Allerdings würde dies die Codegenerierung mit dem aktuellen Ansatz schwerer machen, weshalb ich mich dagegen entschieden habe. Im fall in dem \textsc{Gaalop} keine Optimierungen nutzt müsste dies jedoch relativ einfach möglich sein. Auch die Wahl der maximalen Untermenge ist nicht trivial.



In der Software liegt dieses Produkt $M \cdot P(x,y,z)$ nicht explizit als Matrix-Vektor-Produkt vor, sondern wird als Computational Graph umgesetzt. Es ist theoretisch möglich, $M$ und $P$ aus dem Graphen zu extrahieren, allerdings ist dies relativ aufwendig, da \textsc{Gaalop} den Graphen intern umformt, um ihn zu vereinfachen. Deswegen funktioniert dieser Ansatz besser, wenn Vereinfachung durch Maxima in \textsc{Gaalop} bei der Codegenerierung abgeschaltet wird.
...
Dazu werden alle Nodes aus dem Graph extrahiert die nur von den Variablen x,y oder z abhängen. Da mehrere nodes das gleiche Polynom oder vielfache darstellen können werden die nodes in Äquivalenzklassen eingeteilt, wobei jede Äquivalenzklasse alle Nodes enthält, die ein Polynom oder ein Vielfaches desselben repräsentieren. Dabei wird für jede Äquivalenzklasse die Node mit den wenigsten Vorfahren als Repräsentant gewählt. Diese bilden die Polynome von P. Falls Vereinfachungen in \textsc{Gaalop} deaktiviert ist P nun vollständig. Andernfalls wird P später nachträglich noch erweitert.

Als Nächstes werden die Outputs als Polynome über dem Vorläufigen P erzeugt. Dabei können nicht lineare Terme entstehen. Deshalb werden im nächsten Schritt diese Terme linearisiert und P wird entsprechend erweitert.

Im letzten Schritt werden die nodes in eine Matrix gepackt.
...
Weiterhin kann die Matrix Normal oder dicht gepackt als vec4 hochgeladen werden. Dabei wird jede Zeile mit Nullen gepaddet um eine Länge die ein Vielfaches von 4 ist zu erhalten. Die dicht gepackten Versionen werden dabei als Dense bezeichnet.
...
Außerdem gibt es noch \enquote{row} was dafür steht das nur die erste Zeile der Matrix zur Auswertung genutzt wird (also \((F_R)_1\)). Diese Methode wertet das dotproduct von der ersten Zeile der Matrix mit \(P\) aus.
