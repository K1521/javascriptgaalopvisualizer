...
\chapter{Motivation}
Geometrische Algebra hat den großen Vorteil, dass die algebraischen Ausdrücke üblicherweise geometrischen Objekten und Operationen entsprechen. Deshalb können Algorithmen gut durch Visualisierungen ausgedrückt werden. Auch \textsc{Gaalop} und das web-basierte \textsc{Gaalop}Web nutzen diese Möglichkeit zur Unterstützung von Geometrische Algebra Algorithmen. Gerade bei der Arbeit mit höheren Dimensionen, die in letzter Zeit immer wichtiger wird, weisen diese Visualisierungen allerdings eine Reihe von Nachteilen auf.
...
Aktuelle Methoden können nur Niedrigdimensionale Algebren Visualisieren oder sind langsam.
...
Um ein Objekt \texttt{obj} in der Geometrischen Algebra als Isofläche darzustellen, wird der sogenannte \textbf{IPNS} und \textbf{OPNS} (Inner/Outer Product Null Space) verwendet. Dieser stellt Objekte der geometrischen Algebra als eine implizite Darstellung da.
...
Die Arbeit mit \( f \) ist in vielen Fällen einfacher als mit \( F \). Auch Ganja und \textsc{Gaalop} nutzen die Summe der Quadrate zur Visualisierung.
...
Zum Beispiel enthält \( F \) in der DCGA Polynome vierten Grades, sodass \( f \) in diesem Fall ein Polynom bis zum achten Grad sein kann.
...
Da \textsc{Gaalop} auch die Summe der Quadrate für \( f_{\text{ray}} \) nutzt (soweit ich das aus dem Quellcode lesen kann), muss \( f_{\text{ray}} > 0 \) sein, was bedeutet, dass es nie einen Nullstellenübergang gibt. Der Code sollte trotzdem funktionieren, mit der Einschränkung, dass \texttt{refinement} nie Nullstellen finden sollte.

\subsection{GradientMethod}
Die Gradientmethode erzeugt wie die DiscreteCubeMethod ein dichtes 3D Gitter. Diese Punkte werden dann iterativ mittels eines Gradienten Abstiegsverfahren verfeinert.
...
Die Arbeit soll eine neue Visualisierungsmethode für kontinuierliche Flächen liefern, die in \textsc{Gaalop} und \textsc{Gaalop}Web eingebunden werden kann. Außerdem sollte die software mit hochdimensionaler geometrischer Algebra umgehen können und bietet eine interaktive Darstellung mit niedrigen Laufzeiten.

Dabei übernimmt \textsc{Gaalop} die Umwandlung von ausdrücken in geometrischer Algebra zu Rechengraphen die dann von meiner Software weiterverarbeitet weren.
...
Da OpenGL sowohl zum Numbercrunching als auch zur Visualisierung genutzt werden kann, habe ich mich entschieden WebGL2 zu nutzen was im Prinzip die OpenGL Variante in JavaScript ist. Zudem benötige ich noch eine Library für lineare Algebra wie Matrix Inverse und QR-Zerlegung, wobei ich mich für mathjs entscheiden habe da diese Library weit verbreitet ist. Außerdem hatte ich eine Library zum plotten benötigt. Dabei habe ich mich für Plotty entschieden da es ähnlich zu Pyplot ist, was ich schon kannte.
Der zweite Teil der Software ist ein Plugin für \textsc{Gaalop}, das wie der Rest von \textsc{Gaalop} in Java geschrieben ist.
Zudem habe ich noch ein paar kleine Python Scripts mit Pandas benutzt, um daten auszuwerten.
...
Da das Verfahren eine lineare Näherung nutzt, ist es nur lokal konvergent. Deshalb lohnt es sich eine maximale Schrittweite einzuführen. Diese kann die Stabilität des Verfahrens stark verbessern.
...
Auch hier habe ich eine maximale Schrittweite eingeführt.
...
Ich habe Zwei Varianten des Verfahrens entwickelt, wobei die Zweite Variante einfacher und schneller ist. Dabei basiert die Zweite Variante auf der Matrix Variante die im Kapitel Implementierung eingeführt wird.
%TODO Symbolic link
...
Ein Nachteil des Aberth-Verfahrens ist, dass sie bei mehrfachen Nullstellen deutlich langsamer konvergiert. Da \( f(\text{ray}(t)) \) im Schnittpunkt mit der Oberfläche immer eine doppelte Nullstelle besitzt, konvergiert dieses Verfahren vor allem an den gesuchten reellen Nullstellen langsam. 




Da die komplexen Nullstellen von \( f(\text{ray}(t)) \) stets konjugiert zueinander auftreten, können diese Paare gemeinsam aktualisiert werden. Seien \( z_1 \) und \( z_2 \) ein konjugiertes Paar, so wird zunächst die Update-Formel für \( z_1 \) angewendet, und anschließend einfach \( z_2 = \overline{z_1} \) gesetzt. Dies spart Rechenaufwand da \( f(\text{ray}(t)) \) nur halb so oft per Iteration ausgewertet werden muss.
...
Das Ergebnis des Aberth-Verfahrens sind approximierte Nullstellen \( z_1, \dots, z_n \) von \( f(\text{ray}(t)) \), also:
%\[
...
Ein Strahl \(\text{ray}'\), der \(\text{ray}\) bei \(\text{ray}(a)\) schneidet, hat an diesem Schnittpunkt also nicht unbedingt eine komplexe Nullstelle. Das bedeutet, dass sich die Form des zu visualisierenden Objektes ändern kann, wenn sich die Kamera bewegt.
...
Ein weiterer Nachteil ist, das dieses Verfahren viel stärker vom rauschen von \(p\) beeinflusst wird. Ich habe mich trotzdem für diese Methode entschieden.
...
Das Isolinien Muster erzeugt ein einfaches Gittermuster entlang der drei Achsen. Ähnlich wie bei dem Schachbrett muster wird für den Punkt p geprüft, ob er auf dem Gitter landet und dementsprechend eingefärbt. 

Ich finde das Isolinien muster normalerweise schöner, allerdings hat es ein paar Artefakte was es hässlich macht.
...
Die Nicht auf Raycasting basierenden Methoden Konzentrieren sich darauf graphische Primitive (Punkte, Dreiecke und Linien) zu generieren und diese dann zu visualisieren. Anders als bei \textsc{Gaalop} passiert die Auswertung des Nullraums dabei auf der GPU.   
...
Der Punktwolken-Erzeuger generiert achsenparallele Strahlen und verwendet das Aberth-Verfahren, um die Nullstellen entlang dieser Strahlen zu finden. Er funktioniert im Prinzip genauso wie die Raycasting-Aberth-Verfahren, allerdings mit einer orthografischen Projektion. Da WebGL2 keine Compute-Shader unterstützt, können die Punkte nicht direkt als Array ausgegeben werden (Man kann stattdessen transform feedback nutzen, allerdings hatte ich das zu diesem Zeitpunkt noch nicht implementiert). Stattdessen werden mehrere Output-Texturen verwendet, in denen die Nullstellen als komplexe Zahlen gespeichert werden. Eine Textur kann maximal vier Kanäle enthalten, und es können bis zu vier Output-Texturen verwendet werden. Das bedeutet, dass pro Fragment maximal 16 Floats ausgegeben werden können. Also bis zu 8 komplexe Nullstellen.
...
Dabei muss beachtet werden, dass die meisten Verfahren nur lokal konvergieren. Deshalb funktioniert diese Methode am besten, um das Ergebnis der Intervallarithmetik Voxel zu filtern.
...
Das Kapitel beginnt mit einem Überblick über die Systemarchitektur. Dann folgt im Unterkapitel QR-Zerlegung eine wichtige mathematische Optimierung zum Beschleunigen. Als Nächstes folgt, das Unterkapitel Codegenerierung in dem beschrieben wird, wie man vom \textsc{Gaalop} Output zur Evaluierung von f auf der GPU kommt. Dann erfolgen ein paar Performance Tests die ich zur Analyse und zum Treffen von Designentscheidungen geschrieben habe.
Zum Schluss folgen noch ein paar einfachere Optimierung.
\section{Systemarchitektur}
%mobile
In einem Progrogrammdurchlauf wird immer als Erstes das von \textsc{Gaalop} generierte JSON eingelesen und als Graph deserialisiert. Dieser Graph enthält Definition der Nullräume der zu visualisierenden Objekte. Der Code dafür ist in der Klasse \textsc{Gaalop}Graph.
Dann wird in main für jedes zu visualisierende Objekte die Pipelines erstellt. Dabei steht jede Pipeline für eine Visualisierungsmethode. Die Pipelines werden dann gruppiert nach Objekt im RenderContext gespeichert. Die Pipelines besitzen jeweils ein oder mehrere Shader. Dabei haben die Raycasting Methoden jeweils einen Shader während die Volumenbasierten methoden jeweils mehrere haben. Ein oder mehrere zur generierung der primitive und dann ein weiterer zur visualisierung der primitive.  
Der Shadercode wird mittels Shaderimporter eingelesen. Normalerweise hat glsl keine Imports. Deshalb habe ich Imports implementiert. Alle aktuell genutzten Shader sind in shaderlibv3.
Alle Shader welche den Nullraum auswerten sind nur Templates. Deshalb wird mittels der Codegenerierung Shadercode zur Auswertung des Nullraums aus dem eingelesenen Graph erzeugt und in den Shader eingefügt. Dies passiert in der Methode gencode.

Immer wenn sich Parameter ändern, müssen die primitive für die volumenbasierten Pipelines neu generiert werden und auch für die Raycasting Methoden müssen teile des Graphs neu ausgewertet werden und Parameter auf die entsprechend aktualisiert werden. Dies passiert in der Methode 
...
Dabei kann M direkt aus dem zu visualisierenden Multivektor berechnet werden, während P die Polynome aus \(\texttt{point}(x,y,z)\) darstellt. 
...
In der Software liegt dieses Produkt $M \cdot P(x,y,z)$ nicht explizit als Matrix-Vektor-Produkt vor, sondern wird als Computational Graph umgesetzt. Es ist theoretisch möglich, $M$ und $P$ aus dem Graphen zu extrahieren, allerdings ist dies relativ aufwendig, da P nicht von \textsc{Gaalop} synthetisiert wird.
...
Dabei gilt \(f'(x,y,z)=0 \iff f(x,y,z)=0\). Weiterhin kann auch \(f = P(x, y, z)^\top M' P(x, y, z)\) berechnet werden.
...
Das bedeutet, dass \(grad((F_R)_i)=grad(P_i)\). Bei der Wahl von \((F_R)_i\) zur Nullstellensuche kann dann das \((F_R)_i\) mit dem niedrigsten grad gewählt werden, was die Nullstellensuche theoretisch beschleunigt.
Ich habe die Sortierung implementiert, aber diesen Fakt nicht ausgenutzt da dies die Nullstellensuche im Shader wesentlich komplexer machen würde. In der Praxis benutze ich daher immer \((F_R)_1\) und lade immer das ganze \(R\) (mit Zeilen die vollkommen aus Nullen bestehen) in den Shader.
...
nicht das Quadrat berechnen. In diesem Fall gilt \(f = F\) statt \(f = |F|^2\).
...
Die neue Codegenerierung kann alles generieren, was die alte Codegenerierung generieren konnte. Zusätzlich wird der Code für die Matrix Variante generiert.
...
Dieser Ansatz war relativ unflexibel und unübersichtlich. Anfangs ging es noch, allerdings hatte ich weitere Ideen und habe dann immer mehr Typen hinzugefügt. Allerdings hätte ich wahrscheinlich dabei bleiben sollen, weil mich das neu schreiben viel Zeit gekostet hat. 
...
Teilweise haben Multivektorprodukte Hunderte an Outputs was bedeutet, dass der WebGL2 Compiler ohne Fehlermeldung aufgibt wobei zusätzlich teilweise der WebGL2 Kontext abstürzt. Die Matrix Version der neuen Codegenerierung Fixt dieses Problem.
...
Für die Matrixvariante muss die \(M\) und \(P(x,y,z)\) aus dem Graphen extrahiert werden.

Ich habe zwei Versionen der Matrixextraktion aus dem Graph geschrieben. 


Die alte Version berechnet nur \(M\) und nimmt an, dass \(P\) gegeben ist. Der Nachteil dabei ist, dass \(P\) von der Algebra abhängt was bedeutet, dass für jede Algebra \(P\)  vordefiniert werden müsste. Die neue Version hat dieses Problem nicht und 
baut \(P\) automatisch. 
...
Um diese Polynome in der Polynombasis der Algebra zu repräsentieren, wird mittels einer Basiswechselmatrix von der Monombasis zur Polynombasis \(P\) der Algebra konvertiert. Somit erhält man \(M\).
...
Die nodes werden sortiert nach Anzahl der vorfahren und dann der Reihe nach zu P hinzugefügt, falls sie linear unabhängig von den schon hinzugefügten Polynomen sind. Ich nutze dazu eine modifizierte Version des Modifizerten Gram-Schmidtschen Orthogonalisierungsverfahren.
...
Zwischendurch habe ich einfach eine Monombasis genutzt. Dies funktioniert auch, ist aber langsamer.
...
Um \(R*P(x,y,z)\) für den Matrixansatz auszuwerten wird Matrixmultiplikation in glsl benötigt, die möglichst performant sein muss. Kleine Änderungen im Shadercode machen große Performance Unterschiede.
...
Außerdem gibt es noch \enquote{row} was dafür steht das nur die erste Zeile der Matrix zur Auswertung genutzt wird (also \((F_R)_1\)). Diese Methode wertet das Skalarprodukt von der ersten Zeile der Matrix mit \(P\) aus.
...
Dann gibt es noch horner was eine Auswertung über die mittels der Vandermonde Matrix approximierten Koeffizienten des Polynoms ist. Weiterhin gibt es bei row noch Methoden mit \enquote{copy} im Namen. Diese kopieren die erste Zeile der Matrix in ein Array und benutzen dann dieses Array zur Auswertung.
...
Berechnet wird immer (außer für horner) \(P\) und dann die Multiplikation mit der ersten Zeile der Matrix für \enquote{row}. Für \enquote{sus} wird die Dumme der Quadrate der Multiplikation von \(P\) mit der ganzen Matrix berechnet. Die Matrix ist dabei eine 25x25 Matrix was einer Monombasis vierten Grades entspricht.
...
\subsection{Polynom entlang des Strahls}
...
Die Koeffizienten berechne ich dabei mit einer Vandermonde Matrix.  
...
Dann sind die Nullstellen von \(f_\text{aprox}(\frac{x}{k})\) im Intervall \([-2,2]\). Allerdings ist diese Version nur ein bisschen stabieler weshalb ich sie nicht benutzt habe.
