...
\reviewer{Dietmar Hildenbrand \and Andr\'e Stork}
...
Die Arbeit mit \( f \) ist in vielen Fällen einfacher als mit \( F \), da das ursprüngliche vektorwertige Problem auf die Bestimmung der Nullstellen einer skalarwertigen Funktion reduziert wird. Auch Ganja und \textsc{Gaalop} nutzen die Summe der Quadrate zur Visualisierung.
...
Auch dieses Verfahren hat einen kleinen Nachteil. Durch die Normalisierung ist der erste Schritt immer Länge 1. Da der gesamte abgetastete Raum eine Kantenlänge von 10 hat, ist dieser Schritt relativ groß und der Punkt landet nicht unbedingt in der Nähe des Startpunktes. Außerdem konvergiert das verfahren nicht besonders schnell. Dies ist für die Visualisierung jedoch nicht unbedingt ein Problem. 
...
Ich habe Zwei Varianten des Verfahrens entwickelt, wobei die Zweite Variante einfacher und schneller ist. Dabei basiert die Zweite Variante auf der Matrix Variante die im Kapitel Implementierung eingeführt wird und wird im Abschnitt~\ref{sec:Schnelle_Nullstellensuche} beschrieben.
%TODO Symbolic link
...
Die Prüfung ob die Nulstellen von \((F_R)_i\) auch Nulstellen von \(F_R\) sind ist allerdings komplizierter. Ich habe mich entschieden an der potientiellen Nullstelle einen Levenberg Marquad schritt auszuwerten, um dann zu prüfen ob die länge dieses Schritts unter einem Schwellwert liegt. Alternativ könnte man auch die UDFAprox oder \(F_R\) an der Nullstelle auswerten und testen ob dies unter einem Schwellwert liegt.
...
Dabei passt dieses Verfahren besonders gut zu der im Abschnitt~\ref{sec:Schnelle_Nullstellensuche} beschriebenen Methode da diese mit Polynomen kleineren Gerades arbeitet. Dies macht die Vandermonde Matrix kleiner und das Verfahren stabieler.
