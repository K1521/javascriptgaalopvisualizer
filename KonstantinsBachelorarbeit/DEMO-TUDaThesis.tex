...
Zum Raycasting wird die Aproximation nun einfach als UDF benutzt wie in \ref{sec:Spheretracing}. 
...
Dabei habe ich zusätzlich zur Auswertung des Nullraums weitere Bewertungsfunktionen implementiert. Zum einen die UDF-Aproximation und zum anderen die Länge eines Gauss Newton Schtrittes an dem entsprechenden Punkt.
...
\subsection{INPS/ONPS als Matrix Vektor Darstellung}
...
Deswegen extrahiere ich als \(\mathbf{P}\) ein maximales linear unabhängiges Subset der Polynome aus dem Graphen die nur von \(x,y,z\) abhängen. Dies funktioniert sehr gut, allerdings ist es eine relative komplexe Lösung. Der Vorteil von diesem Ansatz ist, das die Größe des Gleichungssystems so minimiert wird.
...
Die Prüfung, ob die Nullstellen von \((\mathbf{F}_R)_i\) auch Nullstellen von \(\mathbf{F}_R\) sind ist allerdings komplizierter. Ich habe mich entschieden an den potenziellen Nullstellen einen Levenberg Marquad Schritt auszuwerten, um dann zu prüfen, ob die Länge dieses Schritts unter einem Schwellwert liegt. Alternativ habe ich auch eine Version implementiert welche die UDFAprox an den Nullstellen auswerte und teste, ob diese unter einem Schwellwert liegt (Siehe \ref{sec:Nullstellentest}). 
...
Um \(\mathbf{F}_R=R\cdot\mathbf{P}(x,y,z)\) für den Matrixansatz auszuwerten wird Matrixmultiplikation in glsl benötigt, die möglichst performant sein muss. Kleine Änderungen im Shadercode machen große Performance Unterschiede.
...
Die Ergebnisse für row und DCrow sind relativ einfach zu verstehen. Die Approximation durch horner ist am schnellsten, da am wenigsten Multiplikationen benötigt werden. Weiterhin sind die dichten Methoden für row schneller. Das liegt daran, dass für diese dot benutzt werden kann, wodurch weniger Multiplikationen benutzt werden müssen.
...
Leider scheint diese Optimierung sehr numerisch instabil zu sein, was zu einem sehr verrauschten Bild führt. In der Nähe der Kamera ist das Rauschen kleiner, aber immer noch relativ groß. Ich vermutete, dass das daran liegt, das Ich Stützstellen nahe der Kamera gewählt habe.
...
:Pl;     
\end{verbatim}
Der Nachteil dabei ist, dass man die Variablen natürlich nicht mehr einstellen kann. Allerdings ist das Visualisieren so viel schneller, da der erzeugte Graph viel kleiner ist. Deshalb habe ich jeweils zwei Performancemessungen in gaalop gemacht. In meinem Visualisierer ist die statische Szene auch etwas schneller, allerdings ist der Unterschied kleiner, weshalb ich meine Zeitmessungen nur mit der dynamischen Szene gemacht habe.
...
Gaalops default Parameter sind maximal 50 iterationen bei einer Gitter Kantenlänge von 10 und einer Gitter Dichte von 1. (Der Parameter cubeEdgeLength steht auf 5 was allerdings bedeutet, das das Gitter von -5 bis 5 auf allen Achsen ist.) Das sind also maximal \(10^3\cdot 50=50000\) Evaluationen des Nullraums pro Objekt.
...
Dabei wird etwa jeweils ein viertel der Zeit für array Soertierung und Auswertung des Nullraums verbraucht, während die restliche Zeit für die Meshgenerierung gebraucht wird. Das Sortieren ist hierbei scheller, da ich anstatt die Punkte zu sortieren nur den Nullraum sortiere.
...
\begin{figure}
    ...
    \caption{GridVoxel in Meiner Software mit einer Auflösung von 200 Abtastpunkten pro Achse}
...
\begin{figure}
    ...
    \caption{Marching Cubes in meiner Software}
...
Das Aberth-Verfahren ist mit hat auch etwa 25 FPS. In der aktuellen Aberth-Variante berechne ich erst 20 Iterationen des Aberth verfahrens mit angenäherten koeffizienten, und dann nochmal 20 Iterationen mit der original Funktion.
Wenn man statdessen 40 Iterationen der original Funktion und keine iterationen der angenäherten Funktion berechnet hat das verfahren nur 15 FPS.
...
Alle meine Raycasting verfahren haben ein Problem die grüne Ebene darzustellen. Das liegt daran, dass die Ebene eine doppelte Nullstelle hat. Dies macht leider alle meine Verfahren instabiel. Das Aberth Verfahren kann nach dem konvergieren explodieren, und auch die UDF-Aproximation explodiert. 
\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{pictures/doubleroot.png}
    \caption{Ebene visualisiert mit Aberth aus der Nähe. An den schwarzen Pixeln konvergiert das Verfahren nicht. In der Regel wird das Muster schlechter, desto weiter man vom Ursprung entfernt ist. Ich vermute das liegt an schlechter Gleitkommaprezision. }
...
\begin{figure}
    ...
    \caption{Darstellung der komplexen Nullstellen des Aberth verfahren über mehrere iterationen. Hier sieht man wie die Komplexen Nullstellen plötzlich explodieren können. }
...
\begin{figure}
    ...
    \label{fig:doubleroot_rootconvergence}
\end{figure}
\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{pictures/doubleroot_alongray.png}
    \caption{UdfAprox (gestrichelt) und \((\mathbf{F}_R(\mathbf{ray}(a)))_1\) entlang eines Strahls. }
...
Eine weitere möglichkeit ist die erweiterung der Software durch neue Methoden. Allerdings müsste dafür erst festgestellt werden welche Methoden in der Praxis gut sind.
