...
Ein weiterer Nachteil ist, das dieses Verfahren viel stärker vom Rauschen von \(\mathbf{p}\) beeinflusst wird. Ich habe mich trotzdem für diese Methode entschieden.
...
Ich hatte eine vereinfachte Version des Verfahrens zur Demonstration implementiert. Dies war ohne Subdivision implementiert, und machte nur eine Iteration. Allerdings war es zu ähnlich zu einer der Gridbasierten Verfahren, weshalb ich es wieder rausgenommen habe (der Code ist noch da aber nicht in der Gui).
...
\section{Grid basierte Verfahren}
TopGrid ist im Prinzip eine Reimplementierung der Discrete Cube Method aus Gaalop. Dabei wird nicht wiein Gaalop der Schwellwert für \(f\) eingestellt. Stattdessen werden die Punkte anhand der Größe des Nullraums an der jeweiligen Position sortiert, und es werden die Punkte mit den kleinsten Werten entsprechend eines einstellbaren Perzentils dargestellt.
...
Nachdem das Bild vollständig gerendert ist, werden die Diagramme eventuell upgedated.
...
Die Prüfung, ob die Nullstellen von \((\mathbf{F}_R)_i\) auch Nullstellen von \(\mathbf{F}_R\) sind ist allerdings komplizierter. Ich habe mich entschieden an den potenziellen Nullstellen einen Levenberg Marquad Schritt auszuwerten, um dann zu prüfen, ob die Länge dieses Schritts unter einem Schwellwert liegt. Alternativ könnte man auch die UDFAprox oder \(\mathbf{F}_R\) an den Nullstellen auswerten und testen, ob dies unter einem Schwellwert liegt.
...
Die Marching Cubes variante Funktioniert relativ gut. Die Darstellung als Dreiecke statt Punkten ist wesentlich besser. Allerdings gibt es Grenzfälle in denen die Meshgenerierung fehlerhaft ist. Weiterhinn Funktioniert das Verfahren nur für Oberflächen und nicht für Kurven.
...
\begin{figure}
    ...
    \caption{\(f_\text{ray}\) des Torusses und die udf-Approximation entlang eines strahls durch ein lokales maximum. (das maximum in diesem fall liegt in der mitte des rings des torusses.)  }
