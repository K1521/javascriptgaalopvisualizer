...
Zum Beispiel enthält \( F \) in der DCGA (Double Conformal Geometric Algebra) Polynome vierten Grades, sodass \( f \) in diesem Fall ein Polynom bis zum achten Grad sein kann.
...
TopGrid ist im Prinzip eine Reimplementierung der Discrete Cube Method aus Gaalop. Dabei wird nicht der Schwellwert für \(f\) eingestellt. Stattdessen werden die Punkte anhand der Größe des Nullraums an der jeweiligen Position sortiert, und es werden die Punkte mit den kleinsten Werten entsprechend eines einstellbaren Perzentils dargestellt.
Außerdem gibt es noch die GaussNewtonGrid Methode. Diese macht einfach für jeden Punkt im Gitter einen Gauss Newton Schritt und visualisiert die Punkte, bei denen die Länge des Schrittes unter einem Schwellwert liegt.
...
\end{minted}
Diese Methode wertet \(f(\mathbf{ray}(a))\) und
...
Laufzeitanalyse
Gaalop Laufzeitanalyse:
Ray Method mit default parametern
samples 50
357,775s
Während die Ebene angezeigt wird lässt sich die kamera nicht bewegen.
...
Dafür braucht Gaalop 57,926 Sekunden.
Bei gleichen Parametern braucht meine Implementierung hingegen ca. 0,021-0,039 Sekunden (die Zeiten schwanken relativ Stark). Das bedeutet meine Implementation ist ca. 2000-mal schneller.
Da meine Implementation so viel schneller ist, kann man einfach höhere Parameter wählen. Bei 50 Abtastpunkten Pro Achse (also \(50^3*50=6250000\) Evaluationen) braucht meine Implementation 0,189-0,202 Sekunden. Dies ist nicht wesentlich langsamer, obwohl 125mal so viel Nullraumauswertungen gemacht wurden. Das bedeutet, dass der Overhead von WebGL2 hier relativ groß ist.
Gegenüber den anderen iterativen Verfahren ist die Konvergenz der GradientMethod schon relativ gut. Dabei konvergiert GradNewton ein bisschen schneller während Levenberg-Marquad und Newton-Like wesentlich langsamer Konvergieren.
Bei der GradientMethod kann es passieren, dass die Punkte gegenüber den anderen Methoden ungleichmäßiger verteilt sind. Dies muss jedoch nicht unbedingt ein Nachteil sein.

Die Iterativen verfahren sind gegenüber den anderen Verfahren relativ Stabiel und haben wenig Artefakte. Der Nachteil gegenüber der Raycasting Verfahren ist hauptsächlich, dass die Punktwolken nur in Bewegung einen guten 3D Eindruck vermitteln.
...
Gaalop hat standertmäßig 50 Abtastpunkten pro Achse und benötigt 57,926 Sekunden.
TopGrid hingegen benötigt 0.260-0.420 Sekunden. Im Vergleich zu den iterativen Methoden ist das sehr Langsam. Das liegt daran, dass die CPU die meiste Zeit damit beschäftig ist die Punkte zu sortieren.
Ich finde die Ergebnisse der Methode visuell weder in gaalop, noch in meiner Implementation besonders überzeugend.

\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{pictures/TopGrid.png}
    \caption{TopGrid in Meiner Software}

    \label{fig:MyGradientMethod}
\end{figure}
    

Discrete Cube Methode
samples 50
