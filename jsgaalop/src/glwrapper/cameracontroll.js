...
export class Cameracontroll{
    constructor(canvas,gl,_onChange=null){
      ...
      this.cameraPos = new Vector([0,0,-1]);
      this.basecampos =this.cameraPos;
      this.gl=gl;
      this.mouse = {
        x: undefined,
        y: undefined,
    ...
    initeventlisteners() {
        ...
        window.addEventListener('blur', () => this.resetKeys(),true);

        document.addEventListener('change', (event) => {
            if (event.target.tagName === 'SELECT') {
                this.resetKeys()
                // Optional: Fokus explizit zurück aufs Hauptfenster/Canvas
                //window.focus(); 
            }
        });
        document.addEventListener('mousedown', (e) => {
            if (e.target.closest('select')) this.resetKeys();
        });

        
    }
    update(deltatime){
    //if (document.activeElement.tagName === 'SELECT') {this.resetKeys();}
