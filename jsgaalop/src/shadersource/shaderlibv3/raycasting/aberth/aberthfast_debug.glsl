...
void Raymarch(vec3 rayDir, vec3 rayOrigin,out float error,out float xmin,vec2 v_rayDirXY) {
    ...
    /*vec2[NUM_ROOTS] candidates;
    int c=0;
    for(int i = 0; i < NUM_ROOTS; ++i){
        Complex r=roots[i];
        if(r.x<0.)continue;
        float y=susR(rayDir*r.x+rayOrigin);
        if(y<ROOT_ZERRO_THRESHOLD*10.){
            candidates[c++]=r;
        }
    }
    for(int i = 0; i < c; ++i){
        ...
        Complex rm=r;
        for(int j=0;j<10;j++){
            DualComplex ff_=DCrowR(rayDir,rayOrigin,vec2(r.x,0.));
            //r-=ComplexDiv(ff_.xy,ff_.zw);
            if(abs(ff_.x)<abs(rm.y))rm=vec2(r.x,ff_.x);
            r.x-=ff_.x/ff_.z;
    ...
    if(v_rayDirXY.x<0.){
        if(v_rayDirXY.y<0.)debugcolor(vec3(1.,-log2(error)/256.,-log2(imag)/256.));
        else               debugcolor(vec3(0.5,0.5+log2(error)/256.,0.5+log2(imag)/256.));
    }
