...
*/
    ...
    generatecode3() {
        ...
        //issquared t f
        //issingular t f
        "xyzDual fdxyz(vec3 pos) {?}";
        "float f(vec3 pos){?}";
        "DualComplex Complexfda(vec3 rayDir, vec3 rayOrigin,Complex a){?}";
        
        //first_polydegree
        "Complexfdafirst(vec3 rayDir, vec3 rayOrigin,float a)"
        "fdafirst(vec3 rayDir, vec3 rayOrigin,float a)"

        "void DualF(vec3 rayDir, vec3 rayOrigin,float a,out Dual[numoutputs] result) {?}";//ignore
        "Intervall Intervallf(Intervall _V_X,Intervall _V_Y,Intervall _V_Z) {?}";
        "bool evaluatevoxelIntervall3d(x0,y0,z0,delta) {?}";
...
*/
        ...
        return {replacements,nodetosubgraphname};
