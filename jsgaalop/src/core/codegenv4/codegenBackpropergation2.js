        ...
        function parseExpression(node) {
            switch(node.type){
                ...
                case "Pow":
                    ...
                    function powNode(base, exponent) {
                        //generated by chatgpt
                        if (exponent === 0) return ConstNode.one;
                        if (exponent === 1) return base;

                        if (exponent % 2 === 0) {
                            const half = powNode(base, exponent / 2);
                            return new MulNode(half, half);
                        } else {
                            const half = powNode(base, Math.floor(exponent / 2));
                            return new MulNode(base, new MulNode(half, half));
                        }
                    }
...
*/
        ...
        //matrixextractor2.extractbasis2(subgraphxyz);
    ...
    gencode(template){
        for(const [k,v]of this.generatecode2cached().replacements.entries())
...
class matrixextractor2{
    ...
    static extractbasis2(root){
        const ismultiple=matrixextractor2.ismultiple;
        const basisnodes = new Set();
        //basisnodes => nodes depend on xyz and children depend on variables which are not xyz
        //

        //poly=>basispoly
        //const basispolys=[]; //{poly,node}
        //const polys=new Map();
        const basissplitnodes=new Set();//nodes only dependant on x,y,z with children which also depend on other variables
        let polys=new Map();
        root.visitnodesrec((node, parentresults) => {
            if(parentresults.some(x=>x===undefined)){
                for(let i=0;i<parentresults.length;i++)
                    if(parentresults[i]!==undefined)
                        basissplitnodes.add(node.parents[i]);
                return undefined;
            }
        ...
        {
            const cache=new Map();
            for(const n of basissplitnodes)
                n.visitnodesrec((node, parentresults) => {
                    const s=new Set([node]);
                    for(const p of parentresults)for(const x of p)s.add(x);
                    numops.set(node,s.size)
                    return s;
                },cache);
        }
        ...
        //find the poly with least ops
        const basispolys=[{node:ConstNode.one,poly:Poly.convert(1),ops:1}];//list of {node,poly,ops}
        const equivalenceclassindex=new Map();//node=>index of basispolys
        for(const [node,poly]of polys.entries()){
            if(poly==undefined)continue;
            const ops=numops.get(node);
            let found=false;
            for(let i=0;i<basispolys.length;i++){
                const basispoly=basispolys[i];
                if(ismultiple( basispoly.poly,poly)){
                    found=true;
                    equivalenceclassindex.set(node,i);
                    if(ops<basispoly.ops)
                        basispolys[i]={node,poly,ops};
                    break;
                }
            }
            if(!found){
        ...
        root.visitnodesrec((node,parentresults)=>{
            ...
            if(equivalenceclassindex.has(node)){//if the node is part of the basis
                ...
                const multiplyer=ismultiple(poly,basis.poly);
                if(!multiplyer)throw new Error("polys arent multiples");
