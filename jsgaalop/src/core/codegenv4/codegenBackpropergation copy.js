...
class Alg{
    static fold(node){
        if (!(node instanceof ScalarExpressionNode)) return node;
        if (node.parents.length > 0 && node.parents.every(x => x instanceof ScalarConstNode))//constantantfolding
            return new ScalarConstNode(node.eval());
        return node;
    }

    static castToIntervall(x){
        if(x.type==NodeType.Scalar)return (new CastToIntervall(x));
        if(x.type==NodeType.Intervall)return x;
        throw new Error("cant cast");
    }
    static CastToComplex(x){
        if(x.type==NodeType.Scalar)return (new CastToComplex(x));
        if(x.type==NodeType.Complex)return x;
        throw new Error("cant cast");
    }


    static _unifytypes(nodes){
        //scalar->direct return
        //Structural->return
        if(nodes.every(x=>x.type===NodeType.Scalar)||nodes.every(x=>x.type===NodeType.Structural))
            return nodes;
        //Complex,(scalar)->cast return
        if(nodes.every(x=>x.type===NodeType.Scalar||x.type===NodeType.Complex))
            return nodes.map(Alg.CastToComplex);
        //Intervall,(scalar)->cast return
        if(nodes.every(x=>x.type===NodeType.Scalar||x.type===NodeType.Intervall))
            return nodes.map(Alg.castToIntervall);
        throw new Error("couldnt unify");
        //other error->
        /*if(nodes.length==0)return{nodes,type:undefined};
        //scalar->direct return
        //Structural->return
        if(nodes.every(x=>x===NodeType.Scalar)||nodes.every(x=>x===NodeType.Structural))
            return {nodes,type:NodeType.Scalar};
        //Complex,(scalar)->cast return
        if(nodes.every(x=>x===NodeType.Scalar||x===NodeType.Complex))
            return {nodes:nodes.map(x=>(x.type==NodeType.Scalar)?new CastToComplex(x):x),type:NodeType.Complex};
        //Intervall,(scalar)->cast return
        if(nodes.every(x=>x===NodeType.Scalar||x===NodeType.Intervall))
            return {nodes:nodes.map(x=>(x.type==NodeType.Scalar)?new CastToIntervall(x):x),type:NodeType.Intervall};
        //other error->*/
    }
    static add(...parents){//parents {Node[]}
        if (parents.length == 1)return parents[0];
        if (parents.length == 0)return ScalarConstNode.zero;
        parents=Alg._unifytypes(parents);const type=parents[0].type;
        if(type==NodeType.Scalar)return new AddNode(...parents);
        if(type==NodeType.Complex)return new ComplexAddNode(...parents);
        if(type==NodeType.Intervall)return new IntervallAddNode(...parents);
    }
    static mul(){
        if (parents.length == 1)return parents[0];
        if (parents.length == 0)return ScalarConstNode.one;
        parents=Alg._unifytypes(parents);const type=parents[0].type;
        if(type==NodeType.Scalar)return new MulNode(...parents);
        if(type==NodeType.Complex)return new ComplexMulNode(...parents);
        if(type==NodeType.Intervall)return new IntervallMulNode(...parents);
    }
    static sub(a,b){
        parents=Alg._unifytypes(parents);const type=parents[0].type;
        if(type==NodeType.Scalar)return new MulNode(...parents);
...
class ExpressionNode{
    ...
    backpropergation() {
        ...
        const derivs = new Map([[this, ScalarConstNode.one]]);//d this/d...
    ...
    backpropergationwithdefault(){
        ...
        return (x)=>dthisd_.get(x)??ScalarConstNode.zero;
    }
    ...
    forwardpropergation(variable) {
        ...
        const derivs = new Map([[variable, ScalarConstNode.one]]);//d(...)/d(variable)
...
class ComplexExpressionNode extends ExpressionNode {
    constructor(...parents) {
        super(parents,NodeType.Complex);
    }
}


class IntervallExpressionNode extends ExpressionNode {
    constructor(...parents) {
        super(parents,NodeType.Complex);
    }
}

class ScalarExpressionNode extends ExpressionNode {
    constructor(...parents) {
        super(parents,NodeType.Scalar);
    }
}

class ScalarConstNode extends ScalarExpressionNode {
    constructor(value) {
        super([]);
        this.value = value;
    }
    clonewithnewparents(newparents) {
        return new ScalarConstNode(this.value);
    }
    ...
    static one = new ScalarConstNode(1);
    static minusone = new ScalarConstNode(-1);
    static zero = new ScalarConstNode(0);
}

class CastToComplex extends ComplexExpressionNode{
    constructor(real,imag=ScalarConstNode.zero){
        super([real,imag]);
    }
}

class CastToIntervall extends IntervallExpressionNodeExpressionNode{
...
class VarNode extends ExpressionNode {
    constructor(varname,type=NodeType.Scalar) {
        super([],type);
...
class AddNode extends ScalarExpressionNode {    
    compute(parentresults, variables) {
        return parentresults.reduce((a, b) => a + b);
    }
    edgederiv() {
        return this.parents.map(x => ScalarConstNode.one);
    }
}

class MulNode extends ScalarExpressionNode {
...
class DivNode extends ScalarExpressionNode {
    constructor(a, b) {
        super([a, b]);
    }
...
class SubNode extends ScalarExpressionNode {
    ...
    edgederiv() {
        return [ScalarConstNode.one, ScalarConstNode.minusone];
    }
}

class NegNode extends ScalarExpressionNode {
    constructor(x) {
        super([x]);
    }
    ...
    edgederiv() {
        return [ScalarConstNode.minusone];
    }
}
...
function simplify(node) {
    ...
    function normalization(node) {
        ...
        if (node instanceof NegNode)
            return new MulNode(ScalarConstNode.minusone, node.parents[0]);
        if (node instanceof SubNode)
            return new AddNode(node.parents[0], new MulNode(ScalarConstNode.minusone, node.parents[1]));
        if (node instanceof DivNode && node.parents[1] instanceof ScalarConstNode)
            return new MulNode(node.parents[0], new ScalarConstNode(1 / node.parents[1].value));
        if (node instanceof MulNode && node.parents.some(x => x instanceof ScalarConstNode && x.value === 0))
            return ScalarConstNode.zero;
    ...
    node = node.visitnodesrec((node, parentreplacements) => {
        if (node instanceof MulNode || node instanceof AddNode) {
            ...
            //merge costants
            let constantfactors = newparents.filter(x => x instanceof ScalarConstNode);
            newparents = newparents.filter(x => !(x instanceof ScalarConstNode));

            if (constantfactors.length > 0) {
                /** @type {ScalarConstNode} */
                let constantfactor;
                if (constantfactors.length > 1)
                    constantfactor = new ScalarConstNode((new node.constructor(...constantfactors)).eval());
    ...
    node = node.visitnodesrec((node, parentreplacements) => {
        if (node instanceof MulNode && node.parents.some(x => x instanceof ScalarConstNode && x.value == -1)) {//insert neg nodes instead of x*-1
            const constantfactors = node.parents.filter(x => x instanceof ScalarConstNode);
            const newparents = node.parents.filter(x => !(x instanceof ScalarConstNode));
...
class CommonSubexpressionElimination {
    ...
    getsignature(node) {
        ...
        if (node instanceof VarNode) {
            sig += node.varname;
        } else if (node instanceof ScalarConstNode) {
            sig += node.value;
        } else {
...
class BundlenodeNode extends Node {
    constructor(...parents) {
        super(parents,NodeType.Structural);
    }
...
class AssignementNode extends Node {
    constructor(variable, expression) {
        super([variable, expression],NodeType.Structural);
        if(variable.type!=expression.type)throw new Error("Type mismatch");
    }
    compute(parentresults) {
        return parentresults;
    }
...
export class visualizationtargetnode extends Node {
    constructor(nodes, name, color) {
        super(nodes,NodeType.Structural);
        this.color = color;
        this.nodes = nodes,
        this.name = name;
    }
    ...
    generatecode() {
        ...
        
        {
            ...
            const body=new BundlenodeNode(subgraphray.parents.map((resulti,i)=>
                new custumCodeNode(
                    [resulti,d_da.get(resulti)??ScalarConstNode.zero],
...
function makeAssignementGraph(root, inlineConstants = true) {
    ...
    root.copyGraph((originalnode, clone) => {
        ...
        if (inlineConstants && originalnode instanceof ScalarConstNode) return;
        if (!(originalnode instanceof ScalarExpressionNode)) {
...
class Complex{
    ...
    static promote(x) {//lazy chatgpt code
        ...
        if (x instanceof Node) return new Complex(x, new ScalarConstNode.zero);

        // If x is a raw number → wrap in ConstNode
        if (typeof x === "number") return new Complex(new ScalarConstNode(x), new ScalarConstNode.zero);
