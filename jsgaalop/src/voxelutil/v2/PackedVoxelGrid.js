import { gridpacker } from "./gridpacker.js";
...
*/
  ...
  */
  subdivide() {
    ...
    //const dy = step << 10;
    //const dz = step << 20;
    const dx = this.packer.xyztokey(step, 0, 0);
    const dy = this.packer.xyztokey(0, step, 0);
    const dz = this.packer.xyztokey(0, 0, step);
  ...
  getPositions(dx = 0, dy = 0, dz = 0, { format = 'float32' } = {}) {
    ...
    const positionFn = (v) => {
      const [x, y, z] = this.packer.unpackVoxel(v);
      return this.packer.voxelToPosition([x + dxs,y + dys,z + dzs]);
    };
  ...
  getVoxelsXyzFlatUInt16(){
    const len = this.voxels.length;
    const out = new Uint16Array(len * 3);

    for (let i = 0; i < len; i++) {
      const key = this.voxels[i];
      const baseidx = i * 3;
      const [x, y, z] = this.packer.keytoxyz(key);
      out[baseidx + 0] = x;
      out[baseidx + 1] = y;
      out[baseidx + 2] = z;
    }
